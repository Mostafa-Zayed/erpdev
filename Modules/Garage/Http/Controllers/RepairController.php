...
class RepairController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!(auth()->user()->can('superadmin') ||(auth()->user()->can('repair.view')) )) {
        ...
        $customers = Contact::customersDropdown($business_id, false);
          $shipments = Shipment::forDropdown($business_id,  true);
        ...
        $service_staffs = $this->transactionUtil->serviceStaffDropdown($business_id);
          $trafic = TraficResource::forDropdown($business_id, true);
           $campaign = Campaign::forDropdown($business_id, true);
             $city = City::get();
        ...
        return view('repair::repair.index')->with(compact('business_locations', 'customers','shipments','shipping_statuses','order_statuses','trafic','city','campaign','service_staffs', 'repair_status_dropdown', 'sales_representative', 'is_service_staff'));
    }
    ...
    public function create()
    {
        ...
        if (!(auth()->user()->can('superadmin') || (auth()->user()->can('repair.create')))) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function edit($id)
    {
        ...
        if (!(auth()->user()->can('superadmin') || (auth()->user()->can('repair.update')))) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function editRepairStatus($repair_id)
    {
        ...
        if (!(auth()->user()->can('superadmin') || (auth()->user()->can('repair_status.update')))) {
            abort(403, 'Unauthorized action.');
        }
